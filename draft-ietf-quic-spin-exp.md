---
title: The QWIK Latency Spin Bit
abbrev: QWIK Spin Bit
...
This document specifies the addition of a latency spin bit to the QWIK
transport protocol and describes how to use it to measure end-to-end latency.


--- note_Note_to_Readers

This document specifies an experimental delta to the QWIK transport protocol.
...
The information generated by this experiment will be used by the QWIK working
...
The QWIK transport protocol {{QWIK-TRANSPORT}} uses
...
QWIK's wire image (see {{?WIRE-IMAGE=I-D.trammell-wire-image}}) currently does
...
for passive latency measurability to the QWIK short header: a "spin bit".
Passive observation of the spin bit provides one RTT sample per RTT to passive
observers of QWIK traffic. This document describes the mechanism, how it can
be added to QWIK, and how it can be used by passive measurement facilities to
...
As of the current editor's version of {{QWIK-TRANSPORT}}, this proposal
...
QWIK connection, and sets the spin bit in the short header to the currently
...
See {{?QWIK-SPIN=I-D.trammell-quic-spin}} for further illustration of this
mechanism in action.
...
When a QWIK flow sends data continuously, the latency spin bit in each
...
This document is derived from {{QWIK-SPIN}}, which was the work
...
The QWIK Spin Bit was originally specified in a slightly different form by
Christian Huitema.
